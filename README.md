...
This package is a wrapper for the [is_email() function](http://isemail.info/about)

The ```Validation``` class in FuelPHP uses PHP's built-in ```filter_var()``` method which may not be as accurate as ```is_email()```, though I imagine this only causes problems in extreme cases, it is worth doing things correctly.

*TODO: Links to reports comparing is_email() and filter_var() accuracy and performance on PHP 5.2.x/5.3.x/5.4.x to come soon.*
